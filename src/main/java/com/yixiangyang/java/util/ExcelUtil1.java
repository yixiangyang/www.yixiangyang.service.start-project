...
public class ExcelUtil1 {
	...
	private static final Logger LOGGER = LoggerFactory.getLogger(ExcelUtil1.class);
...
//	}
	...
	public static void exportExcel(List<String> headerList, List<Line> lines, String title,String excelName) {
		...
		XSSFRow rowRowName = sheet.createRow(rown);
		// 创建表头
		...
		XSSFCellStyle style1=style1(workbook);
		// 创建excel数据
		for (int i = 0; i < lines.size(); i++) {
			...
			XSSFRow nextrow = sheet.createRow(i+row);
...
// 标题第一行样式
	public static XSSFCellStyle titleStyle(XSSFWorkbook wb) {
		XSSFCellStyle style = wb.createCellStyle();
		style.setAlignment(HorizontalAlignment.CENTER);
		style.setVerticalAlignment(VerticalAlignment.CENTER);
		style.setBottomBorderColor((short) 8);
		style.setBorderBottom(BorderStyle.THIN); // XSSFCellStyle.BORDER_THIN
	...
	// 第二行样式
	public static XSSFCellStyle style(XSSFWorkbook wb) {
		XSSFCellStyle style = wb.createCellStyle();
		style.setAlignment(HorizontalAlignment.CENTER);
		style.setVerticalAlignment(VerticalAlignment.CENTER);
		style.setBottomBorderColor((short) 8);
	...
	// 数据行样式
	public static XSSFCellStyle style1(XSSFWorkbook wb) {
		XSSFCellStyle style1 = wb.createCellStyle();
		style1.setWrapText(true);// 设置自动换行
		style1.setAlignment(HorizontalAlignment.CENTER); // XSSFCellStyle.ALIGN_CENTER
		style1.setVerticalAlignment(VerticalAlignment.CENTER); // XSSFCellStyle.VERTICAL_CENTER // 创建一个居中格式
		style1.setBottomBorderColor((short) 8);
		style1.setBorderBottom(BorderStyle.THIN); // XSSFCellStyle.BORDER_THIN
